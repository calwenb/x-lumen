...
SRC=/wen/project/frontend/xlumen-frontend-admin           # 源码目录（测试/正式共用）
APP=/wen/app/frontend/xlumen-frontend-admin               # 产物目录（nginx root 指向它）
BLOG_URL=http://159.75.6.183                           # 构建期写入 VITE_BLOG_URL（前台入口；无域名用 IP）
...
# 构建期写入"前台入口"地址，编译时打进页面（.env.production 留在仓库里，构建完不用还原）
