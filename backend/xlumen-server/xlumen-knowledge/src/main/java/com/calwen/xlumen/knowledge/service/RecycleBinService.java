...
/**
 * 回收站服务：知识库与知识统一回收站，type=kb|knowledge，deleted_at 降序，
...
public interface RecycleBinService {
    ...
    /**
     * 回收站分页列表：type=kb 查 kb_knowledge_base（status=1）；
    ...
    /**
     * 恢复：kb 恢复 status=0 + deleted_at=null，并发布 KbRecycleStatusEvent(status=0)
    ...
    /**
     * 彻底删除：二次确认参数 confirm 必须等于 "CONFIRM"（否则 409）。
