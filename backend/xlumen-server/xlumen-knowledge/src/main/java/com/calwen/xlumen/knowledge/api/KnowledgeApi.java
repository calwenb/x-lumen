...
/**
 * 知识模块对外接口（BACKEND.md §5.2）：发布即索引与 RAG 检索，
 * 供 ai 模块对话检索编排（M08）与 publishing 发布事件触发（M10 事件由本模块监听消费）。
 * KB-3 扩展知识库/目录能力与可见库集合推导，决策 D13/D16）。
...
public interface KnowledgeApi {
    ...
    /**
     * 索引知识：切片->Embedding->写新索引->检索校验->激活->清理旧版本。
    ...
    /**
     * 查询知识当前索引状态：供 publishing 补跑入口回显结果；未索引返回 null。
     *
     * @param workspaceId 工作空间 ID
     * @param knowledgeId 知识 ID
     * @return 索引状态或 null
    ...
    /**
     * 目录树：parent_id 多级，按名称排序（数据库排序规则）。
     *
     * @param kbId 知识库 ID
    ...
    /**
     * 可见库集合推导，决策 D13）：访客=全平台公开库；库主=公开库 + 自己私有库；
     * V2 增加授权库。公开读、搜索、RAG 检索、知识列表共用，禁止散落重复过滤。
     *
    ...
    /**
     * 回收站库列表：当前空间 status=1（回收站）的库，deleted_at 降序。
    ...
    /**
     * 恢复回收站库：库恢复（status=0+deleted_at=null），连带恢复库内知识
    ...
    /**
     * 彻底删除回收站库：物理删除库并发布 KbPurgedEvent（content 侧级联删知识）。
     *
     * @param workspaceId 工作空间 ID
     * @param kbId        知识库 ID
     */
