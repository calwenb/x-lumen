...
/**
 * 发布即索引事件监听（F-0402，决策 D13）：监听 publishing 发布的进程内 ArticlePublishedEvent，
...
public class ArticlePublishedEventListener {
    ...
    /**
     * 消费文章发布事件，异步执行发布即索引流水线。
     *
     * @param event 文章发布成功事件（含正文快照）
     */
    @EventListener
    public void onArticlePublished(ArticlePublishedEvent event) {
        indexExecutor.execute(() -> {
            try {
                ...
                log.error("发布即索引任务失败：articleId={}", event.getArticleId(), e);
            }
        });
    }
}
