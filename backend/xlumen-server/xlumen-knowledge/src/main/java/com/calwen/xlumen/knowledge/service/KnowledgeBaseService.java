...
/**
 * 知识库服务（决策 D16）：库 CRUD、可见性切换与回收站归属变更，
...
public interface KnowledgeBaseService {
    ...
    /**
     * 创建知识库：归属当前会话空间；名称空间内唯一（uk_kb_ws_name）冲突 409；
    ...
    /**
     * 更新知识库：不存在或跨空间 404；名称修改后仍唯一校验（冲突 409）；
     * 简介/封面空值不覆盖。
     *
     * @param kbId 知识库 ID
    ...
    /**
     * 删除知识库：二次确认参数 confirm 必须等于 "CONFIRM"（否则 409）；
    ...
    /**
     * 空间内知识库列表：status=0（正常）的库，按名称排序（数据库排序规则）。
    ...
    /**
     * 知识库详情：不存在或跨空间返回 null（不抛异常，供 Api 层映射）。
     *
    ...
    /**
     * 知识库详情（按当前会话空间校验跨空间 404 语义的便捷入口）。
     *
     * @param kbId 知识库 ID
     * @return 知识库视图或 null
     */
