-- 40_content.sql：xlumen-content 模块（cnt_ 知识主体/版本/可见性）
...
-- 知识主体（F-0201/F-0202/F-0307）：公开读过滤条件为 status=已发布(6) AND visibility=公开(1)（KB-2 起可见性由知识库决定，本表删除 visibility 列）
CREATE TABLE IF NOT EXISTS `cnt_knowledge` (
    ...
    `category`     VARCHAR(64)  NOT NULL DEFAULT '' COMMENT '分类（公开筛选维度，F-0202；KB-2 起废弃，由 kb_directory 目录树接管）',
    ...
    `visibility`   TINYINT      NOT NULL DEFAULT 1 COMMENT '可见性：1 公开 0 私有（F-0307，私有不进公开列表与搜索；KB-2 起废弃，可见性由知识库决定）',
    ...
    PRIMARY KEY (`id`),
    KEY `idx_knowledge_ws_pub` (`workspace_id`, `status`, `visibility`, `published_at`),
    KEY `idx_knowledge_ws_status` (`workspace_id`, `status`, `updated_at`),
    KEY `idx_knowledge_ws_category` (`workspace_id`, `category`),
