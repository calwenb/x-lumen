-- 20_knowledge.sql：xlumen-knowledge 模块（kb_ 文章知识索引 RAG，发布即索引，决策 D13）
...
CREATE TABLE IF NOT EXISTS `kb_chunk` (
    ...
    PRIMARY KEY (`id`),
    KEY `idx_chunk_article_version` (`workspace_id`, `article_id`, `version`),
    KEY `idx_chunk_article_hash` (`workspace_id`, `article_id`, `content_hash`)
) ENGINE = InnoDB COMMENT ='切片元数据（F-0402/F-0405）';

-- 索引版本与活动指针（F-0403）：同一文章多版本并存，仅一条 ACTIVE 为当前生效索引
CREATE TABLE IF NOT EXISTS `kb_index_version` (
    `id`              BIGINT       NOT NULL COMMENT '主键（雪花 ID）',
    `workspace_id`    BIGINT       NOT NULL COMMENT '工作空间 ID（逻辑外键 iam_workspace.id）',
    `article_id`      BIGINT       NOT NULL COMMENT '文章 ID（逻辑外键 cnt_article.id）',
    `version`         BIGINT       NOT NULL COMMENT '发布版本号（关联 cnt_article.version）',
    ...
    PRIMARY KEY (`id`),
    KEY `idx_index_ws_article` (`workspace_id`, `article_id`),
    KEY `idx_index_ws_article_status` (`workspace_id`, `article_id`, `status`)
