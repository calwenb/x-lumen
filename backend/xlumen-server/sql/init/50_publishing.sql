...
CREATE TABLE IF NOT EXISTS `pub_review` (
    ...
    `knowledge_title`    VARCHAR(200) NOT NULL DEFAULT '' COMMENT '知识标题（冗余展示字段）',
    `version`            BIGINT       NOT NULL COMMENT '知识版本号（提交审核时快照）',
...
CREATE TABLE IF NOT EXISTS `pub_release` (
    ...
    `knowledge_id`    BIGINT       NOT NULL COMMENT '知识 ID（逻辑外键 cnt_knowledge.id）',
    `knowledge_title` VARCHAR(200) NOT NULL DEFAULT '' COMMENT '知识标题（冗余展示字段）',
    `version`         BIGINT       NOT NULL COMMENT '知识版本号（发布时快照）',
    ...
    `created_at`      DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_release_ws_knowledge_version` (`workspace_id`, `knowledge_id`, `version`)
) ENGINE = InnoDB COMMENT ='知识发布记录（F-0904/F-0905）';
