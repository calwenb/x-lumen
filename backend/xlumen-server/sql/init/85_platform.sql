-- 85_platform.sql：xlumen-identity 模块平台治理域（plt_ 前缀）——M13 落地 审计日志
...
-- 平台操作审计日志：operator_id 可空（系统/定时任务触发）；KEY (workspace_id, created_at)
CREATE TABLE IF NOT EXISTS `plt_activity_log` (
    ...
    `action`        VARCHAR(64) NOT NULL COMMENT '操作类型（如 REVIEW_REJECT/KNOWLEDGE_PUBLISH/WORKSPACE_SETTINGS_UPDATE）',
    `target_type`   VARCHAR(32) NOT NULL DEFAULT '' COMMENT '目标类型（如 REVIEW/KNOWLEDGE/WORKSPACE）',
