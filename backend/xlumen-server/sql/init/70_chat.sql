...
-- M08（~）落地：chat_conversation（会话）+ chat_message（消息）。
-- 消息 citations_json 为检索证据快照（SearchResultDTO 数组），支持引用溯源跳转原文。

USE `xlumen_dev`;
SET NAMES utf8mb4;

-- AI 对话会话：一个工作空间下的多轮对话容器，标题取首条提问截断。
...
-- AI 对话消息：role=USER|ASSISTANT|TOOL；citations_json 为引用证据快照。
-- 加工具轨迹三列：assistant 行存 tool_calls_json（ToolCall 数组快照），tool 行存 tool_call_id/tool_name。
