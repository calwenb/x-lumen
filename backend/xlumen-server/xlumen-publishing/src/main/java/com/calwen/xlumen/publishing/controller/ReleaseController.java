...
/**
 * 发布接口（B05 内容管理后台）：需登录访问；工作空间上下文取自可信会话（WorkspaceContext）。
 * 发布状态流转与幂等集中在 ReleaseService（禁 Controller 判断状态）。
 * 016：补 @PreAuthorize 职责分离（发布/下架属内容管理域，需 OWNER 角色），
 * 新增下架端点。
 *
 * @author calwen
 * @date 2026/8/13
...
public class ReleaseController {
    ...
    /** 发布记录列表：查询参数由 PageQueryDTO 自动绑定。 */
    @GetMapping
    ...
    /** 下架知识：仅已发布可下架，迁移 UNPUBLISHED(8) 并出索引。 */
