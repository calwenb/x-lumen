...
/**
 * 知识数据访问（cnt_knowledge）：仅 content 模块内部使用（BACKEND.md §5.1）。
 *
 * @author calwen
 * @date 2026/8/12
 */
...
public interface KnowledgeMapper extends BaseMapper<KnowledgeEntity> {
    ...
    /**
     * 阅读量自增：独立 SQL 避免读改写竞态（BACKEND.md §11 并发）。
     *
     * @param id          知识 ID
    ...
    /**
     * 从回收站恢复：仅清除回收站标记并留空删除时间，内容归属（kb_id/directory_id）不动；
    ...
    /**
     * 标签聚合：JSON_TABLE 展开 tags 数组统计，按数量降序；仅统计已发布且不在回收站的知识
