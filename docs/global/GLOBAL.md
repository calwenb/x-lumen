...
内容闭环一句话：**输入 → AI 写作 → 审校 → 发布前自动 AI 审核 → 发布（发布即索引）→ 反馈 → 更新**（详见 PRODUCT.md 第 3 节）。AI 知识对话为产品核心能力线之一。

13 个产品模块一句话概览（功能编号与阶段以 PRODUCT.md 第 5 节总表为唯一来源，总表统计：MVP 47 / V2 27 / V3 25，V3 含 6 项暂缓）：
...
| 九 | 审核与发布 | 8 状态内容状态机、AI 自动审核发布、发布幂等与回滚下架（F-0901~F-0907） | MVP/V2 |
...
| 十三 | 技术基础设施 | Redis 缓存、异步任务、统一响应、事件解耦、深浅色主题切换与开发端口守卫（F-1301~F-1307） | MVP/V2/V3 |
...
| [上线部署文档](../deploy/DEPLOY.md) | 生产部署：fat jar + 前端静态产物、Nginx 反代（含 SSE）、systemd、验证与回滚 | 部署操作唯一来源 |
...
│  ├─ xlumen-boot/src/main/resources/application-*.yml   # 环境配置（决策 D29/D30：仅 application-demo.yml 模板入库；dev/test/prod 含真实密钥不入库，服务器放 jar 同级 config/）
