...
│  ├─ blog/                # 知识列表、知识详情、搜索目录标签、知识库页、发现页、关于（F-0201~0203/0208）
│  ├─ content/             # 创作中心：知识列表、Markdown 编辑器、知识库/目录选择（F-0301/0302/0307）
│  ├─ ai/                  # AI 创作任务：写作、审校、进度（F-0601~0604）
│  ├─ knowledge/           # 知识库管理、目录管理、回收站、知识索引状态、检索测试（F-0308/0309、F-0402~0405）
│  ├─ publishing/          # 审核中心、发布管理（F-0901~0905）
│  ├─ chat/                # AI小光对话菜单页 B00、知识级问答（F-0701/0702）
...
| 领域跨页面状态 | 领域 Store | 当前知识、审核筛选条件 |
| 全局会话状态 | 全局 Store | 当前用户、工作空间、权限、未读数 |
| 服务端事实 | API 查询结果 | 知识状态、知识版本、任务状态 |
...
- 注册即建空间（D9 多用户知识平台默认单空间）；博主与读者均在博客前台登录，博主登录后额外获得创作中心；管理员在管理后台登录，非管理员角色拒绝进入（F-0101/0103）。
...
- 作者默认不能直接发布自己的知识；编辑不能审核自己提交的知识（F-0903）；个人空间可关闭强制审核（决策 D9）。
- 知识库可见性由后端强制执行：私有库知识对外不可见（404 语义），前端不得以隐藏入口代替权限控制。
...
| 端到端 | Playwright | 登录、创作、审校、审核、发布和知识库管理闭环 |
...
- 提交信息遵循 Conventional Commits，例如 `feat(content): 支持知识提交审核`。
