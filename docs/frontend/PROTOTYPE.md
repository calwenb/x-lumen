...
- MVP：B00~B04、B08~B13、B16、B20~B22（博客）、A01~A04（管理后台）、D01/D02（AI 对话）；V2：B05、B07、B14、B15、B17~B19、B24、A05~A07；V3：B06。
...
角色/入口：访客从 B01 库 badge、B03 结果或 B21 进入；私有库仅库主可进入，库主额外看到编辑入口。关键状态：库资料加载/空库空态；目录加载失败可重试。核心交互：左侧多级目录树（按目录名排序，每目录显示知识数）；右侧知识列表（更新时间倒序；进入目录后按创建时间正序；置顶知识排最前，V2 F-0310）；[+ 新建目录] 与 [✎编辑库资料] 仅库主可见。
