...
| 应用 | 面向角色 | 默认入口 | 核心任务 |
| --- | --- | --- | --- |
| 博客前台（xlumen-frontend-blog，:6010） | 访客（VISITOR）、登录读者、博主（OWNER） | 知识列表页（首页） | 阅读、搜索、评论、点赞、纠错、AI 对话、知识库浏览；博主额外获得创作中心：创作、审校、审核、发布、知识库与目录管理 |
| 管理后台（xlumen-frontend-admin，:6011） | 仅管理员（OWNER/ADMIN） | 管理后台登录页 | 空间/成员/角色、模型配置、审计日志等配置管理 |
...
- MVP：B00~B04、B08~B13、B16、B20~B23（博客）、A01~A04（管理后台）、D01/D02（AI 对话）；V2：B07、B14、B15、B17~B19、A05~A07；V3：B05、B06、B24。
...
角色/入口：访客从首页搜索框或目录/标签链接进入。关键状态：搜索中骨架；无结果空态（关键词建议）；错误可重试。核心交互：关键词与目录/标签组合筛选；命中高亮；滚动触底自动加载更多；搜索结果仅含可见知识（按知识库可见性过滤，F-0307）。
...
| --- | --- | --- | --- | --- |
| B05 | RSS 订阅（V3，F-0205） | 访客/订阅器，页脚 | 订阅源可用/失效 | 输出 RSS 2.0 订阅链接 |
| B06 | 知识归档（V3，F-0207） | 访客，导航入口 | 按年/月分组 | 时间轴归档、展开月份列表 |
| B07 | 访客 AI 助手·小光（V2，F-0703） | 访客，全站悬浮位 | 加载/回答/引用展开 | 基于公开库已发布知识回答，回复带引用可展开（F-0405） |
| B24 | 作者主页（V3，F-0209） | 访客，从库页/发现页作者名进入 | 加载/空态 | 作者简介、公开库列表、知识时间流（更新时间倒序）；按人浏览入口 |

> 注：V2/V3 范围经决策 D18（2026/8/20）与 D19（2026-08-22）两次调整（见 PRODUCT §5 阶段标记说明）：D19 按「个人使用 × 访客/面试官浏览可见」评分重划为 V2 27 项 / V3 25 项（含 6 项暂缓），原 V3 的 AI 功能并入 V2、B05（RSS）/B24（作者主页）随 F-0205/F-0209 移至 V3；9 项新候选已登记总表（F-0216~F-0220/F-0706/F-0707/F-0809/F-1005），多复用既有页面（问搜一体/语义搜索=B04 搜索、导读浮层/术语解释/图片讲解=详情页 B03 系列、@小光=评论区块、相关追问/转草稿=B00/D01、站点 AI 导游=全站浮层），实施时按对应模块既有页面模式补充；尚有独立页面的 V3 为 B05 知识归档（F-0207）、B05 RSS（F-0205）、B24 作者主页（F-0209）与无独立页面的 F-1305 全文搜索。

### 7.7 知识库页与发现页（B20/B21，MVP · F-0208）

#### B20 知识库页

角色/入口：访客从 B01 库 badge、B03 结果或 B21 进入；私有库仅库主可进入，库主额外看到编辑入口。关键状态：库资料加载/空库空态；目录加载失败可重试。核心交互：左侧多级目录树（按目录名排序，每目录显示知识数）；右侧知识列表（更新时间倒序；进入目录后按创建时间正序；置顶知识排最前，V3 F-0310）；[+ 新建目录] 与 [✎编辑库资料] 仅库主可见；**目录树节点右键菜单（F-0312，仅库主）：重命名/删除（沿用目录删除连带规则，二次确认）/新增子目录，「全部知识」树根右键新增根目录，与 B01 左栏目录树行为一致**。

```text
┌──────────────────────────────────────────────────────────────────────────────┐
│ ← 返回    【技术周刊】 🔓公开   关注✚(V3 暂缓)    知识 86 · 目录 7                  │
│   专注前端与 AI 工程的实践笔记……                                             │
├──────────────────────────────┬───────────────────────────────────────────────┤
...
角色/入口：登录用户，头像下拉 [我的收藏]（`meta.authenticated`，路由 `/favorites`）。关键状态：加载骨架；无收藏空态（引导去逛知识库）；列表失败可重试。核心交互：收藏知识卡片流（标题/作者/所属库/摘要/收藏时间，点击进 B02）；卡片 [取消收藏] toggle 即时移除；滚动触底自动追加，显示加载中/已到底。
...
角色/入口：博主从创作中心"知识"导航进入；编辑器支持深链接。关键状态：列表加载/空/错误/无权限/滚动追加；编辑器保存中/已保存/保存失败/版本冲突四态；状态机 8 状态展示（F-0901）。核心交互：状态/知识库/目录筛选；**发布时选择目标知识库与目录（F-0307 库级可见性）**；同库内可换目录，跨库不提供移动（仅复制到新库或重新发布）；自动保存每 10 秒或失焦触发、内容未变化不发请求（F-0302）；冲突禁止静默覆盖，提供查看服务端版本/复制本地/基于最新继续编辑；点击「发布」后先二次确认，再由 Reviewer 自动检查，确认文案明确提示 AI 审核。
...
编辑器：左侧知识信息（知识库/目录/标签/封面），中间 Markdown 编辑区（大纲/所见即所得/源码切换），右侧按需打开 AI 面板（复用 B11/AI 增值面板）。发布区明确展示发布时间，留空表示审核通过后立即发布，选择未来时间表示定时发布；新建知识无需先单独保存，点击发布后先二次确认，系统自动保存标题、正文、知识库、目录和标签的最新值，再执行 Reviewer。审核期间锁定编辑并显示进度状态；error/失败时阻断并逐条展示位置、依据、建议，warning/info 也展示详情并等待作者确认。操作层级固定为次按钮「保存」、主按钮「发布」。

#### B11 AI 创作任务页（MVP · F-0601~F-0604）

角色/入口：博主从工作台/知识页发起创作任务进入。关键状态：任务 Queued/Running/WaitingApproval/Completed/Failed/Cancelled；流式内容与博主正在编辑区域冲突时不得覆盖，生成结果进入候选版本；关闭页面任务继续执行，返回时从最后事件序号恢复。核心交互：输入主题/草稿/完整知识 → AI 直接输出完整知识（F-0601）；大纲可选确认（F-0602，V2）；知识增强写作检索可见知识库作参考（F-0603，V2）；审校报告按严重度/位置/证据/建议列出（F-0604），严重问题未处理默认禁止发布；[重新审校] [保存修改] [AI 审核并发布]。
...
#### B12 审核中心（暂时隐藏，代码保留 · F-0902~F-0904）

角色/入口：当前无前台入口，旧页面和接口保留以便回退。新发布统一走 F-0907 自动 AI 审核。
...
角色/入口：博主。关键状态：AI 审核中/建议待确认/阻断/已发布/已下架；定时发布到期执行幂等检查，重复任务不产生重复公开版本（F-0905）。核心交互：编辑器点击「发布」后先二次确认并自动执行 Reviewer；error 或任务失败阻断，同时向客户展示高风险位置、依据和建议，warning/info 展示建议并由作者确认继续；立即/定时发布与下架回滚仍展示版本、目标库和影响范围。
...
├──────────────────────────────────────┬───────────────────────────────────────┤
│ 最近发布记录                         │ 自动审核状态                         │
│ 知识、版本、知识库、状态、发布时间    │ 审核中/建议/阻断/已发布                │
