...
（空--2026/8/17 15:10 清空：BUG-002/003/004/005 经用户要求统一修复并验证通过，编号不回收，详见 CHANGELOG 2026/8/17 条目。遗留运维事项：①后端需重启使 Milvus 探测修复生效；②存量 3 版本索引需逐篇调用 `POST /api/v1/knowledge/{knowledgeId}/reindex` 补跑向量）
