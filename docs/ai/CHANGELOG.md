...
## 2026/8/20 15:52 · Codex（IDEA-007/008 后端落地）

> 影响文档：docs/backend/BACKEND.md、docs/global/GLOBAL.md、README.md · 决策摘要：D9、D16

审核发布新增独立 AI Reviewer 自动审核发布接口：审核任务完成且无 `error` 问题才允许发布，阻断/失败自动退回草稿；重复发布按知识版本返回已有发布记录。新增开发环境可选端口守卫 `XLUMEN_DEV_PORT_GUARD=true`，启动时发现端口占用会交互确认并结束占用进程，生产环境默认关闭。
