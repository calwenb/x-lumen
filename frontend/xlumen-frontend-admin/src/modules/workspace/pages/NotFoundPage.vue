...
// 404 页：访问不存在的路由时兜底，给出返回空间设置入口，避免空白页。
</script>

<template>
  <main class="notfound">
    <span class="notfound__code" aria-hidden="true">404</span>
    <h1 class="notfound__title">页面不存在</h1>
    <p class="notfound__text">你访问的地址不存在或已被移除，请检查地址或返回空间设置。</p>
    <router-link class="notfound__home" :to="{ name: 'settings' }">
      返回空间设置
    </router-link>
  </main>
</template>
...
.notfound__title {
  margin: var(--xl-space-6) 0 var(--xl-space-3);
